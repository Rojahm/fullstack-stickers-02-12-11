...
async function StickerPackSinglePage({ params }) {
  ...
  //   `${process.env.NEXT_PUBLIC_SRV_URL}/allstickerPack/${title}`
  // ).then((res) => res.json());
  // // const title = params.categoryName;
  ...
  return (
    <div>
      {title}
      <HeaderStickerPack color={params.color} textColor={"black"} />
    </div>
    // <>
    ...
    //       <div className="text-[#814997] capitalize">/ {pack._id}</div>
    //     </div>
    //     {pack ? (
